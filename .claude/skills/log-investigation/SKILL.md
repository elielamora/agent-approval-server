...
description: >-
  ...
  data, enumerates tool_input fields per tool type, cross-references with ui.ts
  makeCodeBlock/makeCard rendering logic, and reports fields the UI silently discards.
---
...
Read `ui.ts` `makeCodeBlock` (around line 258) and `makeCard` (around line 436). For each tool type, note exactly which `tool_input` fields are read vs. silently ignored.
