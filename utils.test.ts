...
import { stableStringify, buildExplainPrompt, buildFocusScript, allowResponse, denyResponse, findGitRoot } from './utils'
...
describe('buildExplainPrompt', () => {
  ...
  test('Write truncates content at 3000 chars', () => {
    ...
    const prompt = buildExplainPrompt({ tool_name: 'Write', tool_input: { file_path: 'f.ts', content } })
...
describe('buildFocusScript', () => {
  ...
  test('Ghostty without cwd returns simple activate', () => {
    const script = buildFocusScript({ terminal_info: { ghostty_resources_dir: '/opt/ghostty' }, cwd: '' })
    expect(script).toBe('tell application "Ghostty" to activate')
  })

  test('Ghostty with cwd', () => {
    const script = buildFocusScript({ terminal_info: { ghostty_resources_dir: '/opt/ghostty' }, cwd: '/home/user/proj' })
  ...
  test('VSCode with cwd', () => {
    const script = buildFocusScript({ terminal_info: { term_program: 'vscode' }, cwd: '/Users/me/project' })
