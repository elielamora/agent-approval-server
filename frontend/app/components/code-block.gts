...
type DisplayKind =
  ...
  | { kind: 'two-part'; headerHtml: SafeString; bodyHtml: SafeString; bodyLang: string }
  | {
      kind: 'code';
      lang: string;
      html: SafeString;
...
function highlight(text: string, lang: string): SafeString {
  try {
    return htmlSafe(hljs.highlight(text, { language: lang, ignoreIllegals: true }).value);
  } catch {
...
function fileParts(fp: string): { dir: string; base: string } {
  ...
  return idx >= 0 ? { dir: fp.slice(0, idx + 1), base: fp.slice(idx + 1) } : { dir: '', base: fp };
}
...
export default class CodeBlock extends Component<Sig> {
  get display(): DisplayKind {
    ...
    if (item.tool_name === 'Write') {
      ...
      return { kind: 'code', lang, html: highlight(content, lang), filePath: fp, fileDir: dir, fileBase: base, description: '' };
    }
    ...
    if (item.tool_name === 'Read') {
      ...
      return { kind: 'code', lang: 'plaintext', html: highlight(text, 'plaintext'), filePath: fp, fileDir: dir, fileBase: base, description: '' };
    }
    ...
    if (item.tool_name === 'Glob') {
      ...
      return { kind: 'code', lang: 'plaintext', html: highlight(pattern, 'plaintext'), filePath: fp, fileDir: dir, fileBase: base, description: '' };
    }
    ...
    if (item.tool_name === 'WebFetch') {
      ...
      return { kind: 'code', lang: 'plaintext', html: highlight(url, 'plaintext'), filePath: '', fileDir: '', fileBase: '', description: '' };
    }

    if (item.tool_name === 'WebSearch') {
      const query = asString(item.tool_input?.query);
      return { kind: 'code', lang: 'plaintext', html: highlight(query, 'plaintext'), filePath: '', fileDir: '', fileBase: '', description: '' };
    }
    ...
    if (item.tool_name && parseMcpToolName(item.tool_name)) {
      ...
      return { kind: 'code', lang: 'json', html: highlight(text, 'json'), filePath: '', fileDir: '', fileBase: '', description: '' };
    }

    if (item.tool_name === 'ExitPlanMode') {
      const plan = asString(item.tool_input?.plan);
      return { kind: 'code', lang: 'markdown', html: highlight(plan, 'markdown'), filePath: '', fileDir: '', fileBase: '', description: '' };
    }
  ...
  get commitSubjectParts() {
    ...
    if (d.kind !== 'git-commit') return null;
    const { subject } = d.info;
    const ccMatch = subject.match(/^(\w+)(\([^)]+\))?(!)?: /);
    if (ccMatch) {
      return { prefix: ccMatch[0], prefixType: ccMatch[1], rest: subject.slice(ccMatch[0].length) };
    }
    return { prefix: null, prefixType: null, rest: subject };
  }

  <template>
    {{#let this.display as |d|}}
      {{#if (eq d.kind "diff")}}
        <DiffBlock @item={{@item}} />
      {{else if (eq d.kind "git-commit")}}
        <div class="git-commit-block">
          <pre><code class="hljs language-bash">{{d.preambleHtml}}</code></pre>
          <div class="commit-message">
            <div class="commit-subject">
              {{#if this.commitSubjectParts.prefix}}
                <span class="commit-type commit-type-{{this.commitSubjectParts.prefixType}}">{{this.commitSubjectParts.prefix}}</span>{{this.commitSubjectParts.rest}}
              {{else}}
                {{this.commitSubjectParts.rest}}
              {{/if}}
            </div>
            {{#if d.info.body}}
              <div class="commit-body">{{d.info.body}}</div>
            {{/if}}
            {{#each d.info.trailers as |trailer|}}
              <div class="commit-trailer">{{trailer}}</div>
            {{/each}}
          </div>
        </div>
      {{else if (eq d.kind "two-part")}}
        <div class="two-part-block">
          <pre><code class="hljs language-bash">{{d.headerHtml}}</code></pre>
          <pre><code class="hljs language-{{d.bodyLang}}">{{d.bodyHtml}}</code></pre>
        </div>
      {{else if (eq d.kind "code")}}
        {{#if d.description}}
          <div class="bash-description">{{d.description}}</div>
        {{/if}}
        {{#if d.filePath}}
          <div class="file-path">
            {{#if d.fileDir}}<span class="file-path-dir">{{d.fileDir}}</span>{{/if}}
            <span class="file-path-base">{{d.fileBase}}</span>
          </div>
        {{/if}}
        <pre><code class="hljs language-{{d.lang}}">{{d.html}}</code></pre>
      {{/if}}
    {{/let}}
  </template>
}
